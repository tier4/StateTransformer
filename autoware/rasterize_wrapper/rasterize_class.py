...
class RasterizeWrapper:
    ...
    def _world_to_image(self, x, y, range_m):
        # 自車両基準の相対座標
        dx = x - self.ego_x
        dy = y - self.ego_y
        ...
        # 画像座標に変換
        x_img = int(x_norm * self.width)
        y_img = int(y_norm * self.height)

        # 画像のY座標は上下が反転しているため補正
        return x_img, self.height - y_img
    
    def _world_length_to_pixel(self, length_m, range_m):
        """
        ...
        pixels_per_meter = self.width / (2 * range_m)
        return int(length_m * pixels_per_meter)

    def rasterize_map(self, range_m):
        ...
        return channels
    def rasterize_rosbag(self, keyframe, before_keyframe, after_keyframe, range_m):
        ...
        return other_agent_channels, ego_agent_channels, context_actions, trajectory_labels, (self.ego_x, self.ego_y, self.ego_yaw, self.ego_bbox_length, self.ego_bbox_width, self.ego_bbox_height)
...
if __name__ == "__main__":
    rasterizer = RasterizeWrapper(osm_file_path="/home/acf15382lp/projects/datasets/lanelet2_map.osm", 
                                  dbdir="/home/acf15382lp/projects/datasets/rosbag")
    range_m = 30
    other_agent_channels, ego_agent_channels, context_actions, trajectory_labels = rasterizer.rasterize_rosbag(keyframe=50, before_keyframe=10, after_keyframe=80, range_m=range_m)
    print(rasterizer.ego_x, rasterizer.ego_y, rasterizer.ego_yaw, rasterizer.ego_bbox_length, rasterizer.ego_bbox_width, rasterizer.ego_bbox_height)
    save_channels_as_png(other_agent_channels, "other_agent", "hoge")
    save_channels_as_png(ego_agent_channels, "ego_agent", "hoge")

    imgs = rasterizer.rasterize_map(range_m=range_m)

    save_channels_as_png(imgs["ego_road"], "map_ego_road", "fuga")
    save_channels_as_png(imgs["other_road"], "map_other_road", "fuga")
    save_channels_as_png(imgs["traffic_sign"], "map_traffic_sign", "fuga")
    save_channels_as_png(imgs["traffic_light"], "map_traffic_light", "fuga")
    save_channels_as_png(imgs["regulatory_element"], "map_regulatory_element", "fuga")
    save_channels_as_png(imgs["right_of_way"], "map_right_of_way", "fuga")

    # 使用例
    input_folder = "/home/acf15382lp/projects/STR2/rasterize_wrapper/hoge"
    output_filename = "rosbag_animation.gif"
    create_gif_from_images(input_folder, output_filename)

    # map_*.pngからもGIFを作成する場合
    # create_gif_from_images(input_folder, "map_animation.gif", pattern="map_*.png")
